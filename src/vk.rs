...
impl Softmax for VkBackend {
    fn softmax_rows(&mut self, rows: usize, cols: usize, x: &mut [f32]) -> anyhow::Result<()> {
        ...
        // ---- guards
        assert!(rows > 0 && cols > 0, "softmax_rows: zero dims");
        assert_eq!(x.len(), rows * cols, "softmax_rows: x len != rows*cols");

        // ---- load shader
        let mut f = File::open("shaders/softmax_rows.spv")?;
        ...
        // ---- pipeline layout (push constants: 2 x u32 = 8 bytes)
        let pc_range = vk::PushConstantRange {
            stage_flags: vk::ShaderStageFlags::COMPUTE,
            offset: 0,
            size: 8,
        ...
        // ---- buffer
        let bytes_x = (x.len() * std::mem::size_of::<f32>()) as u64;
        let (buf_x, mem_x) = self.alloc_host_buffer(bytes_x)?;
        self.write_buffer_pod(mem_x, x)?;

        // ---- descriptor set { binding 0: X buffer }
        ...
        // ---- record
        let begin = vk::CommandBufferBeginInfo::builder();
        let pc_data = [rows as u32, cols as u32]; // exactly 8 bytes
        unsafe {
            self.device.begin_command_buffer(cmd, &begin)?;
            self.device.cmd_bind_pipeline(cmd, vk::PipelineBindPoint::COMPUTE, pipeline);
            self.device.cmd_bind_descriptor_sets(
            ...
            // one WORKGROUP per row (matches row = gl_WorkGroupID.x)
            self.device.cmd_dispatch(cmd, rows as u32, 1, 1);
